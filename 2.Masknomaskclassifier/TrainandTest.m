...
%% Create HoG training features from Training data
...
maskClassifier = fitcecoc(trainingFeatures,trainingLabel);
save('utilities\maskClassifier.mat','maskClassifier','-v7.3');
%% Test video
% vidobj =  vision.VideoFileReader('data\nomask.mp4');%Name of the video
% viewer = vision.DeployableVideoPlayer;
% load(fullfile("utilities","detectorYoloV2.mat"));
% load(fullfile("utilities","faceClassifier1.mat"));
% position =  [1 20];
% value = 'Processing...';
% counter=1;
% personLabel={};
...
%        personLabel = predict(maskClassifier,queryFeatures);
%        counter=counter-1;
%        end
%     end
